...
private suspend fun opprettEnkelValkeyKlient(host: String, port: Int, useTLS: Boolean): GlideClient {
...
private suspend fun opprettClusterValkeyKlient(nodesList: List<Pair<String, Int>>, useTLS: Boolean): GlideClusterClient {
...
suspend fun opprettValkeyKlient(host: String, port: Int, useTLS: Boolean): BaseClient {
    ...
    return try {
        ...
        opprettClusterValkeyKlient(listOf(Pair(host, port)), useTLS)
    } catch (e: Exception) {
        try {
            log.warn("Feil under opprettelse av cluster-valkey-klient, forsøker å opprette enkel valkey-klient")
            opprettEnkelValkeyKlient(host, port, useTLS)
        } catch (e: Exception) {
